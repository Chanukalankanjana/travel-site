        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex-shrink-0">
            <a href="/" className="flex items-center space-x-2">
              ...
              <span className={`text-xl font-bold ${isScrolled ? "text-gray-900" : "text-white"}`}>Ceylon Vacations</span>
            </a>
          </div>
                  ...
                  {languages.map((language) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 flex items-center space-x-2 ${
                        currentLanguage.code === language.code ? "bg-emerald-50 text-emerald-700" : "text-gray-700"
                      }`}
                    >
            ...
            <nav className="flex flex-col space-y-2">
              {navItems.map((item) => (
                <a
                  ...
                  className="text-gray-700 hover:text-emerald-600 hover:bg-gray-50 px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200"
                >
                  {t(item.key)}
                </a>
              ))}
